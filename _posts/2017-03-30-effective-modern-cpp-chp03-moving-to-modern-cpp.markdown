...
* 特殊成员函数，是指c++会自动生成的成员函数。c++98中包括默认的构造函数、析构函数、拷贝构造函数、(拷贝)赋值运算符。c++11中因为引入了搬移操作，又新加入了默认的搬移构造函数和搬移赋值运算符。

* c++98中的`Rule of Three`在c++11仍然有效：在一个类里，两种拷贝操作(构造函数和赋值运算符)和析构函数必须同时声明。

* 搬移操作的默认函数的自动生成需要满足三个条件：
  1. 没有声明拷贝构造函数和(拷贝)赋值运算符。
  2. 没有声明搬移构造函数和搬移赋值运算符。
  3. 没有声明析构函数。
  > 主要是因为以上三类函数都涉及资源的管理，一旦我们自己声明了这三类函数，默认的搬移操作就很可能是有问题的了。

* 使用`=default`可以要求编译器使用默认的特殊成员函数实现。

* 默认的搬移操作和拷贝操作是互斥的。一旦我们主动声明了搬移操作，那编译器就不会生成默认的拷贝构造函数和(拷贝)赋值运算符。一旦我们主动声明了拷贝操作，则默认的搬移操作函数也不会生成。
