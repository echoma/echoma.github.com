...
![循环神经网络示意图](http://upload-images.jianshu.io/upload_images/2256672-cf18bb1f06e750a4.jpg)

这一讲后面的推导和实现代码我都没看了，放弃了。只记录下重点概念。

`循环神经网络(RNN)`非常适合自然语言处理领域，它对特征进行建模，可以对概率进行学习。

这里介绍了个训练算法，叫做BPTT，我完全没去看了。

里面内容似乎对自然语言先进行了分词，完成分词后的句子才是向量的，才可以进行学习。

### 1.1.6 第六讲 长短时记忆网络

放弃看详尽内容了，只看了开头的介绍，似乎是解决了RNN的梯度爆炸问题。文中说这是RNN最流行的变体，都用来处理序列（这是说语言是序列结构的吗？）。

### 1.1.7 第7讲 递归神经网络

递归神经网络的缩写也是RNN。它能搞好的解决语义歧义的问题。但是要求输入是树结构，极少在现实中使用。

## 1.2 GOOGLE的《机器学习速成课程》

* 同事推荐的，GOOGLE做的课程，主要针对希望学习TensorFlow的用户，但会讲解很多基础知识。
* 课程需要翻墙看：[《机器学习速成课程》](https://developers.google.com/machine-learning/crash-course/)
* 学习时间为：20180328开始，目前还在学习中。
* 下面记录学习过程中的一些疑问和思考。
...
主要就是了解pandas库：
  ...
  * 点进去，发现google的课程用了个叫做`ipynb`的格式，自行搜索了解了下，是一种的用于python的交互式笔记格式，超好用，这个课程里的代码都可以现场执行并打印结果（每段代码前面有个播放按钮，点击运行）。
  * pandas这个库真他娘好用，相见恨晚。

其他会用到第三方库教程里说可以用到的时候再来看：[第三方python库](https://developers.google.com/machine-learning/crash-course/prereqs-and-prework#-python-)

### 1.2.2 机器学习简介+框架处理

* 术语：监督式机器学习、标签、特征、样本({特征+标签}、{特征+无标签})、模型(将样本映射到预测标签的方法)
...
* 从最简单的模型入手。
* 根据房间面积预测房价，使用线性方程预测。关键词：`线性回归`、`权重`、`偏差`
* 评估模型的误差，常见的用方差。关键词：`损失函数`、`平方损失(L2损失)`、`均方误差(MSE)`。
